...
% 这里不再关心输入是如何生成的，只消费标准化后的 external_input。
...
% 当前净失衡已由 main.m 根据“负荷数据.xlsx”生成并完成 10 s 插值，
% 且起始值天然为 0，因此这里直接使用，不再额外乘启动包络。
disturbance.net_imbalance = system.external_input.net_imbalance_mw(:);
end
