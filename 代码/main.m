...
disp("Simulation completed. Results have been saved to simulation_results.mat, simulation_results.xlsx, and the figures folder.");
end
...
% 当前输入文件为一行 8760 点全年小时负荷数据：
% A1 为文字“负荷数据”，B1:... 为按时间顺序排列的 8760 个数值。
% 程序直接读取第一个工作表，不再要求 net_imbalance 工作表或固定列名。
system.input_excel.file_path = fullfile(fileparts(mfilename("fullpath")), "负荷数据.xlsx");
system.input_excel.source_interval_s = 3600.0;  % 原始负荷数据为 1 h/点
system.input_excel.day_index = 1;               % 使用全年数据的第 1 天，可改为 1~365
system.input_excel.load_scale_to_mw = 1.0;      % 若 Excel 已经是 MW，则保持 1.0
...
% - 第一个工作表（当前为 Sheet1）；
% - A1 为文字标题；
% - 其余单元格为按小时顺序排列的全年负荷数据（8760 点）。
%
% 为保持原 4 h / 10 s 闭环仿真框架不变，本函数会：
% 1) 自动提取 Excel 中全部有限数值；
% 2) 按小时负荷数据进行线性插值，得到 10 s 分辨率的背景负荷；
% 3) 以仿真起始时刻负荷为运行基准，将负荷变化量作为净失衡输入。
%
% 这样无需修改原 Excel 的工作表名称、行列方向或增加 time_s /
% disturbance_mw 等列。

input_file = system.input_excel.file_path;
if ~exist(input_file, "file")
    error("未找到负荷数据文件：%s", input_file);
end

raw_matrix = readmatrix(input_file);
load_values = raw_matrix(isfinite(raw_matrix));
load_values = load_values(:) .* system.input_excel.load_scale_to_mw;

if isempty(load_values)
    error("负荷数据文件中未读取到有效数值：%s", input_file);
end

% 当前文件应为全年小时数据（365*24 = 8760 点）。
% 不强制限定必须恰好 8760 点，但至少要覆盖所选仿真时间段。
source_dt = system.input_excel.source_interval_s;
source_hour_axis = (0:numel(load_values)-1)' .* source_dt / 3600;

start_abs_hour = (system.input_excel.day_index - 1) * 24 + system.sim_start_hour;
target_time_s = (0:system.num_steps-1)' .* system.dt;
target_abs_hour = start_abs_hour + target_time_s / 3600;

if start_abs_hour < source_hour_axis(1) || target_abs_hour(end) > source_hour_axis(end)
    error(["所选仿真时段超出 Excel 数据范围。当前共有 %d 个小时数据点，" ...
        "day_index=%d，sim_start_hour=%.2f，sim_hours=%.2f。"], ...
        numel(load_values), system.input_excel.day_index, ...
        system.sim_start_hour, system.sim_hours);
end

% 将小时级负荷线性插值到 10 s 仿真时间尺度。
background_load = interp1(source_hour_axis, load_values, target_abs_hour, "linear");

% 由于当前 Excel 只有负荷序列、没有计划功率或预测误差序列，
% 这里以仿真起始负荷作为运行基准，将相对基准的负荷变化量视为净失衡。
reference_load = background_load(1);
net_imbalance = background_load - reference_load;

background_ramp = [0; diff(background_load)] ./ max(system.dt, 1e-9);

system.external_input = struct();
system.external_input.time = target_time_s(:);
system.external_input.hour_axis = system.sim_start_hour + target_time_s(:) / 3600;
system.external_input.background_load_mw = background_load(:);
system.external_input.background_ramp_mw_per_s = background_ramp(:);
system.external_input.residual_mw = net_imbalance(:);
system.external_input.net_imbalance_mw = net_imbalance(:);
system.external_input.reference_load_mw = reference_load;
system.external_input.data_source = input_file;
system.external_input.source_points = numel(load_values);
system.external_input.source_interval_s = source_dt;
system.external_input.day_index = system.input_excel.day_index;

fprintf("Loaded %d hourly load points from %s.\n", numel(load_values), input_file);
fprintf("Simulation window: day %d, %.2f-%.2f h; reference load = %.4f MW.\n", ...
    system.input_excel.day_index, system.sim_start_hour, ...
    system.sim_start_hour + system.sim_hours, reference_load);
end
...
station.station_names = ["Station 1", "Station 2", "Station 3"];
...
metrics.p95_abs_freq_dev_hz = local_percentile(abs(results.freq_dev), 95);
...
function q = local_percentile(x, percentile_value)
% 无需 Statistics and Machine Learning Toolbox 的百分位数计算。
x = sort(x(:));
x = x(isfinite(x));
if isempty(x)
    q = NaN;
...
p = min(max(percentile_value, 0), 100) / 100;
...
if lower_idx == upper_idx
    ...
    q = x(lower_idx) * (1 - weight) + x(upper_idx) * weight;
end
end
