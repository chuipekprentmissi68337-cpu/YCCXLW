...
figure("Name", "EV and Thermal-Unit Regulation Sharing");
window_minutes = 15;
...
figure("Name", "Available Capacity Comparison");
bar_data = [simple_up, proposed_up];
station_labels = compose("Station %d", 1:numel(station.station_names));
bar(categorical(station_labels), bar_data, "grouped");
grid on;
...
hour_axis = build_hour_axis(scenarios.proposed.time, system);
figure("Name", "EV Regulation Output");
...
hour_axis = build_hour_axis(scenarios.proposed.time, system);
figure("Name", "Thermal-Unit Regulation Output");
subplot(3, 1, 1);
